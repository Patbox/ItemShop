            ...
            if(a.cache[req.body.paymentId]) {
                if(config.important.purchases == "json") {
                    ...
                    pool.getConnection(function(err, connection) {
                        ...
                        const params = [cache[req.body.paymentId].name, cache[req.body.paymentId].service, req.body.paymentId, cache[req.body.paymentId].price];
                ...
                for(const number in services[cache[req.body.paymentId].service].commands) {
                    ...
                    if(config.important.commands == "pterodactyl") {
                        await a.nodeactyl.sendCommand(config.important.pterodactyl.server, command).then((sendcommand) => {
